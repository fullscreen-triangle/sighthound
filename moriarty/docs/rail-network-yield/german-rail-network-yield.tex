...
\begin{figure*}[!htbp]
    ...
    \caption{\textbf{Occupancy--Closure--Clearing equivalence (Theorem~\ref{thm:trinity}) on a two-hop corridor.}
    (\textbf{A})~Yield of the $58$ task-delivering single-section neighbours of the brute-force optimum, sorted; every neighbour lies strictly below the optimum (dashed line), so the optimum admits no improving reassignment---it is closed.
    (\textbf{B})~Per-section separation costs $\sep(\sigma)$ recovered from the yield functional: the essential downstream section carries a large separation cost while the substitutable upstream section carries a smaller one, and these equal the clearing prices by construction.
    (\textbf{C})~Closure gap and maximum clearing-deviation profit at the optimum, both at the floor: no reassignment and no participant deviation gains more than the resolution-floor threshold $\blocktime$ (in yield units).
    (\textbf{D})~Three-dimensional yield surface over the (upstream-vehicle, downstream-vehicle) speed choices at fixed carried set; the maximum sits at the fast/fast corner, and the closed optimum (marked) coincides with it.}\label{fig:val_equivalence}
\end{figure*}
...
\begin{figure*}[!htbp]
    ...
    \caption{\textbf{Necessity of a common quantum (Proposition~\ref{prop:quantum}).}
    (\textbf{A})~Predicted equivalence slack versus the largest of the three split constants across trials; the slack tracks the maximum, and is minimal only when all three coincide.
    (\textbf{B})~Price-versus-separation band width against the algorithmic--economic split $|\blocktime^{\mathrm{alg}}-\blocktime^{\mathrm{econ}}|$; the band grows linearly and vanishes at zero split.
    (\textbf{C})~Measured closure gap and clearing-deviation profit per trial, each staying within its own threshold, showing where the equivalence survives and where it degrades.
    (\textbf{D})~Three-dimensional surface of equivalence slack over the (algorithmic, economic) multiplier plane at fixed physical floor; the unique minimum lies on the diagonal where the two are equal.}\label{fig:val_quantum}
\end{figure*}

\subsection{Forced speed and comparative-advantage sorting}

\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_2_forced_speed.png}
    \caption{\textbf{Forced optimal speed (Theorem~\ref{thm:forcedspeed}, Def.~\ref{def:optspeed}).}
    (\textbf{A})~Yield ratio as a function of traversal speed for several energy weights $\lambda$; each curve is single-peaked, and the peak is the section-optimal speed $v^\star$.
    (\textbf{B})~$v^\star$ against $\lambda$: at $\lambda=0$ it sits at the capability ceiling, and it decreases monotonically as energy cost binds, becoming an interior optimum---the signature of the strictly convex energy term.
    (\textbf{C})~Decomposition of the denominator into block-second and energy--wear components across speed, showing the trade-off whose balance point is $v^\star$.
    (\textbf{D})~Three-dimensional surface of the yield ratio over the (speed, $\lambda$) plane; the ridge of maxima traces $v^\star(\lambda)$ from the ceiling down into the interior.}\label{fig:val_forced_speed}
\end{figure*}

Experiment~2 sweeps traversal speed and energy weight for a fixed section and vehicle. The yield ratio is single-peaked in speed at every $\lambda$ (Figure~\ref{fig:val_forced_speed}), the peak $v^\star$ sits at the capability ceiling when energy is free and moves strictly into the interior as $\lambda$ grows, and each peak is verified a local maximum---exactly the content of Theorem~\ref{thm:forcedspeed} and the role of strict convexity in Def.~\ref{def:optspeed}. Experiment~3 instantiates the Nuremberg--Berlin corridor of Section~\ref{sec:example} and computes per-section yield densities for a high-capability and a regional vehicle. On the two $300$~km/h new-build sections the high-capability vehicle strictly dominates; on the two speed-limited sections neither has an advantage and the densities coincide. The fleet thus sorts itself onto sections by comparative advantage in block-seconds with no assignment rule imposed, confirming Theorem~\ref{thm:sorting} (Figure~\ref{fig:val_sorting}).

\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_3_sorting.png}
    \caption{\textbf{Comparative-advantage sorting on the Nuremberg--Berlin corridor (Theorem~\ref{thm:sorting}).}
    (\textbf{A})~Per-section yield density of the high-capability and regional vehicles; the high-capability vehicle dominates on the $300$~km/h sections and ties on the speed-limited ones.
    (\textbf{B})~Block-seconds consumed per section by each vehicle class; the gap on the fast sections is what the density advantage rests on.
    (\textbf{C})~Density advantage $D_H-D_R$ per section against the section speed limit, crossing to zero exactly where the civil limit falls to the regional ceiling.
    (\textbf{D})~Three-dimensional surface of yield density over the (section speed limit, vehicle capability) plane; the winning vehicle is the higher point above each section, and the two surfaces meet where capability ceases to help.}\label{fig:val_sorting}
\end{figure*}

\subsection{Liveness under target-relative descent}

\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/panel_4_liveness.png}
    \caption{\textbf{Liveness under greedy target-ward routing (Theorem~\ref{thm:liveness}, Remark~\ref{rem:target_relative}).}
    (\textbf{A})~Residual distance of each item against round; every trajectory is monotone non-increasing and reaches zero within a finite number of rounds.
    (\textbf{B})~Total undrained buffer pressure across the network against round, decaying to zero as all buffers drain.
    (\textbf{C})~Number of items settled against round, rising to the full population.
    (\textbf{D})~Three-dimensional surface of residual distance over the (item, round) plane; the surface descends everywhere to the zero floor, visualising simultaneous draining of all buffers on a directed, target-descendable corridor.}\label{fig:val_liveness}
\end{figure*}

Experiment~4 simulates greedy target-ward routing (Def.~\ref{def:routing}) on a directed corridor with a feeder branch. The corridor is not universally gradient-connected---it is one-way---but it is target-descendable, so Remark~\ref{rem:target_relative} applies. Every item's residual distance is monotone non-increasing and reaches zero within a finite number of rounds, and the total undrained pressure decays to zero (Figure~\ref{fig:val_liveness}). This corroborates Theorem~\ref{thm:liveness} under precisely the weaker hypothesis identified in Remark~\ref{rem:target_relative}, and confirms that the directedness of realistic corridors is compatible with the liveness guarantee.
