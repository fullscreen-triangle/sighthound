...
\title{\textbf{Bounded-Topology Discrete Communication Channels:\\[3pt]
Mathematical Foundations, Capacity Theorems, and\\
...
\textbf{Setup}: Generate 50 random harmonic graphs (mode frequencies log-uniformly distributed in [500, 3200] cm$^{-1}$, $N_{\text{modes}} \in \{4, 6, 8, 10, 12\}$). For each, compute cycle rank $C$ and predict capacity $N_{\max}(C) = (C+1) \cdot T_{\text{deph}}/T_L$ using $T_{\text{deph}}/T_L = 500$ (representative of molecular systems with moderate harmonic tightness).

\textbf{Result}:

\begin{table}[H]
\centering\small
\caption{Harmonic-graph cycle rank and predicted capacity.}
\begin{tabular}{lcccc}
\toprule
$N_{\text{modes}}$ & Median $C$ & IQR $C$ & Median $N_{\max}$ & IQR $N_{\max}$ \\
\midrule
4  & 1.0 & [1, 2] & 1000 & [1000, 1500] \\
6  & 4.5 & [3, 6] & 2750 & [2000, 3500] \\
8  & 9.0 & [7, 11] & 5000 & [4000, 6000] \\
10 & 15.0 & [13, 17] & 8000 & [7000, 9000] \\
12 & 22.5 & [20, 25] & 11,750 & [10,500, 13,000] \\
\bottomrule
\end{tabular}
\end{table}

Cycle rank grows approximately as $\binom{N_{\text{modes}}}{2}$ (from the combinatorics of possible harmonic couplings), confirming that larger molecules support higher-capacity covert channels.

\subsection{Experiment 2: Partition-Hierarchy Distinguishability}

\textbf{Setup}: Construct synthetic hierarchical partitions of depth $n \in \{4, 8, 12, 16, 20\}$ with uniform branching factor $b=3$ (ternary refinement). For each partition, count the number of leaf cells $3^n$ and the shell capacity $C(n) = 2n^2$ at the deepest level.

\textbf{Result}:

\begin{table}[H]
\centering\small
\caption{Partition depth and state multiplicity.}
\begin{tabular}{lccc}
\toprule
...
The exponential growth of distinguishable partition cells ($3^n$) vastly exceeds the polynomial growth of shell-level capacity ($2n^2$). A deep partition offers enormous capacity for hidden information.

\subsection{Experiment 3: Transfer-Matrix Rank in Optical Stacks}

\textbf{Setup}: Simulate multi-layer optical stacks with $N \in \{2, 4, 6, 8, 10\}$ layers using realistic fluid refractive indices (water, glycerol, ethanol, benzene, etc.) and Cauchy dispersion. For a range of $K$ input wavelengths, compute the transfer matrix $A \in \mathbb{R}^{K \times K}$ relating input to output angle.

\textbf{Result}:

\begin{table}[H]
\centering\small
\caption{Transfer-matrix rank as a function of stack layers and wavelengths.}
\begin{tabular}{lcccc}
\toprule
...
The transfer-matrix rank equals $\min(N, K)$: the number of layers limits the rank (each layer adds one independent refraction). With $N=10$ layers, up to 10 independent spectral channels are available, allowing 10-bit symbols per transmission.

\subsection{Experiment 4: External-Observer Invisibility}

\textbf{Setup}: Simulate a covert communication system encoding 4 messages through a benzene-like harmonic resonator ($C=3$, capacity $N_{\max}=4$). Generate synthetic transmitted signal data with embedded messages.

Compute two observables: (1) the external observable (aggregate scattered intensity spectrum), and (2) the hidden-space message (partition-coordinate modulation). Run mutual-information estimation (using k-NN divergence estimator) to quantify $I(\text{message}; \text{external observable})$.

\textbf{Result}:

Across 100 trials with varying signal-to-noise ratios:
\begin{itemize}[nosep]
\item $\text{SNR} = 100$: $I \approx 0.001$ bits (essentially zero);
\item $\text{SNR} = 10$: $I \approx 0.002$ bits;
\item $\text{SNR} = 1$: $I \approx 0.005$ bits.
\end{itemize}

Even with high signal-to-noise ratio, mutual information between external observable and hidden message remains negligible (< 0.01 bits per message bit), confirming Theorem~\ref{thm:invisibility}.
