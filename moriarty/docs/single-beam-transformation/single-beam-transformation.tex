...
\begin{figure*}[!htbp]
    ...
    \caption{\textbf{Cross-domain coherence between frequency bands decays exponentially with spectral separation, demonstrating that spatial decompositions represent the same signal under different metric structures.}
    ...
    (\textbf{C})~Three-dimensional coherence surface over 2D parameter space (separation $\Delta_{1}$ and $\Delta_{2}$, each 0--10 GHz) with coherence as height. Surface exhibits smooth exponential decay in both directions, reaching minimum $\approx 0.0001$ at opposite corners ($\Delta_{1} = \Delta_{2} = 10$ GHz).
