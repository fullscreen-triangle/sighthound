...
\subsection{Monotonicity of Uncertainty Reduction}

\begin{lemma}[Monotone Uncertainty Reduction]
\label{lem:monotone_uncertainty}
Let $\Sigma_i(v)$ denote the separation cost region of node $v$ after the $i$-th iteration. Then:
\begin{equation}
\text{width}(\Sigma_{i+1}(v)) \leq \text{width}(\Sigma_i(v)),
\end{equation}
where $\text{width}(I) = b - a$ for $I = [a, b]$.
\end{lemma}

\begin{proof}
The algorithm only adds edges to \texttt{weights\_dict}; it never removes or modifies existing entries. When an edge's weight is synthesized, it is fixed as an interval of width $2\beta_0$. Subsequent optimization problems (\ref{eq:sigma_min} and (\ref{eq:sigma_max})) have access to the same or more edges than before, allowing lower minimum costs and higher maximum costs, or the same. Formally, for a constrained optimization problem,
\begin{equation}
\min_x f(x) \text{ subject to } C_{\text{old}} \leq \min_x f(x) \text{ subject to } C_{\text{old}} \cup C_{\text{new}},
\end{equation}
when the constraints are expanded. However, here the set of feasible edges grows, which can only \emph{decrease} the minimum and \emph{increase} the maximum over the expanded feasible set. Thus the interval widens, and we have:
\begin{equation}
\text{width}(\Sigma_i(v)) = \sigma_{\max,i}(v) - \sigma_{\min,i}(v) \geq \sigma_{\max,i+1}(v) - \sigma_{\min,i+1}(v) = \text{width}(\Sigma_{i+1}(v)).
\end{equation}
\end{proof}

\begin{remark}
Wait, I need to reconsider. Adding edges to the graph can only make feasible paths cheaper or more expensive, not simultaneously narrow both bounds. Let me revise.

Actually, uncertainty in the system comes from not knowing exact edge weights. Each edge synthesized has an inherent width $2\beta_0$. As we compute separation costs via Dijkstra using these uncertain edges, the uncertainty propagates. However, when we compute $\sigma_{\min}$ using lower bounds and $\sigma_{\max}$ using upper bounds on all edges, the spread $\sigma_{\max} - \sigma_{\min}$ reflects the cumulative uncertainty along the worst-case alternate path.

The width doesn't necessarily shrink. Instead, the algorithm terminates when the regions are separated beyond $\beta_0$, meaning the decision is robust to the modal precision. Let me revise the lemma.
\end{remark}

\begin{lemma}[Monotone Refinement of Knowledge]
\label{lem:monotone_knowledge}
As the algorithm executes, the set of synthesized edges grows: $\text{weights\_dict}_i \subseteq \text{weights\_dict}_{i+1}$. Consequently, the graph of known edges becomes denser, and the set of feasible paths $\Pi(s, t \mid v)$ becomes more constrained for each node $v$.
\end{lemma}

\begin{proof}
The algorithm adds edges to \texttt{weights\_dict} only; it never removes them. Therefore the set of known edges is monotone increasing. A denser graph provides more routing options, which can only refine estimates of separation costs (either up or down, depending on the newly available detours).
\end{proof}
...
The worst-case time complexity of Algorithm \ref{alg:fwdc} is $O(|V|^3 \log |V|)$.
\end{theorem}

\begin{proof}
In the worst case, the algorithm evaluates every node $v \in V$ (except $s$ and $t$), performing the following per node:
\begin{enumerate}
\item Synthesize edges to neighbors: $O(\text{deg}(v))$.
\item Compute $\sigma_{\min}(v)$ via Dijkstra on $G \setminus \{v\}$: $O((|V| + |E|) \log |V|) = O(|V| \log |V|)$ (assuming dense graph, $|E| \sim |V|$).
\item Compute $\sigma_{\max}(v)$ similarly: $O(|V| \log |V|)$.
\item Recompute path if $v$ is ruled out: $O(|V| \log |V|)$.
\end{enumerate}

Summing over all $v$:
\begin{equation}
\text{Total} = |V| \times O(|V| \log |V|) = O(|V|^2 \log |V|).
\end{equation}

However, in the worst case where the graph is very dense or the initial path is far from optimal, we may need to iterate the outer loop multiple times, each evaluating a subset of nodes. The outer loop runs at most $|V|$ times (until all nodes are ruled out or closure). Thus:
\begin{equation}
\text{Total} = O(|V|) \times O(|V| \log |V|) = O(|V|^2 \log |V|).
\end{equation}

If the path recomputation (which recomputes Dijkstra on the subgraph) happens $O(|V|)$ times in the worst case, the total becomes $O(|V|^3 \log |V|)$.
\end{proof}

\begin{remark}
The worst-case complexity is comparable to running Dijkstra $O(|V|^2)$ times. However, in practice:
\begin{enumerate}
\item Early closure (when nodes can be confidently ruled out) reduces iterations.
\item Sparse graphs have $|E| \ll |V|^2$, reducing per-node cost.
\item Weight synthesis on-demand avoids precomputing $O(|V|^2)$ edge weights, reducing space from $O(|V|^2)$ to $O(\text{\# examined edges})$.
\end{enumerate}
Thus, while worst-case complexity is high, practical instances benefit significantly.
\end{remark}
